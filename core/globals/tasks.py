...
def check_before_send(**kwargs) -> None:
    ...
    if (is_body_safe(data.get('body')) and 
        is_user_safe(data.get('sender')) and 
        is_bypass_maximum_mails_per_day(data.get('sender')) and 
        is_attachment_safe(attachments)): 
    ...
    if data['status'] == MAIL_STATUS.okay.value:
        ...
        send_notification(
            to_user_id=str(real_time_data.get('reciver').id),
            **serializer.data
        )